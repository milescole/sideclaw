...
uv run sideclaw cron history
uv run sideclaw cron history --job-id <job-id>
```
...
- `agent`: model, workspace, token/temperature defaults, memory window, provider selection (`auto`/`anthropic`/`openai`/`ollama`/`openrouter`), streaming toggle, retry settings (`retry_max`, `retry_base_delay`, `retry_max_delay`)
